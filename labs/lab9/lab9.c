...
TCP, SOCK_STREAM and not SOCK_DGRAM
3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code? The data is read from STDIN_FILNO or
the terminal input. read(STDIN_FILNO, ...)
4. How does the client program end? How can you tell that in the code?
Stops when input is eof or >=1 byte, closes socket and exits, end of code
handling and fucntions used
*/
#include <arpa/inet.h>
#include <stdio.h>
#include <stdlib.h>
